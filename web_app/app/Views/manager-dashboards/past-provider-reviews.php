...
<body>
    ...
    <div class="container">
        <nav>
            <div class="side_navbar">
                ...
                <a class="active" href="<?php echo site_url('providerReviews'); ?>">Past Provider Reviews</a>
                    ...
                    <tbody>
                        <?php if (!empty($reviews)) : ?>
                            <?php foreach ($reviews as $index => $review) : ?>
                                <tr>
                                    <td><?= $index + 1; ?></td>
                                    <td class="hidden-column"><?= $review['provider_rating_id']; ?></td>
                                    <td><?= esc($review['provider_name']); ?></td>
                                    <td><?= esc($review['service_name']); ?></td>
                                    <td><?= esc($review['review_text']); ?></td>
                                    <td><?= $review['review_sentiment'] ? 'Positive' : 'Negative'; ?></td>
                                    <td><?= esc($review['reviewed_on']); ?></td>
                                    <td>
                                        <button class="enlist-button" onclick="appealReview('<?= $review['provider_rating_id']; ?>')">Appeal</button>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else : ?>
    ...
    <form id="appealForm" action="<?php echo base_url('providerSelect') ?>" method="post" style="display:none;">
        <input type="hidden" name="provider_rating_id" id="reviewToAppeal" value="">
    </form>
    ...
    <script>
        ...
        function appealReview(provider_rating_id) {
            document.getElementById('reviewToAppeal').value = provider_rating_id;
